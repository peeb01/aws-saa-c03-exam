...
*   **Policy Evaluation Logic (กฎเหล็กการประเมินสิทธิ์):**
    1.  **Explicit Deny:** มีกฎปฏิเสธชัดเจน -> บล็อกทันทีเด็ดขาด (ชนะทุก Allow)
...
*   **Permissions Boundaries:** นโยบายขั้นสูงสุดที่กำหนดไว้เพื่อควบคุมสิทธิ์ของ IAM User หรือ Role ไม่ให้สามารถยกระดับหรือมีสิทธิ์เกินกว่าเกณฑ์นี้ได้ แม้ว่าจะไปได้รับนโยบาย Administrator Access ในภายหลังก็ตาม (มักใช้ควบคุมสิทธิ์ที่แอดมินย่อยไปแจกจ่ายให้คนอื่น)
...
*   **Trap Warnings:**
    *   *Access Key leaks:* หากโจทย์ระบุว่าเจอ Access Key หลุดไปสาธารณะ (e.g. GitHub) หรือต้องให้โปรแกรมเข้าถึง AWS ห้ามเก็บ Access Key ไว้ในไฟล์โปรแกรมหรือเครื่อง EC2 ให้เปิดใช้งาน **IAM Role** และสวมสิทธิ์ให้กับอินสแตนซ์ผ่าน Instance Profile เสมอ
...
*   **Trap Warnings:**
    *   *KMS Key Policies:* IAM Policy เพียงอย่างเดียวไม่สามารถปลดสิทธิ์ KMS CMK ได้ หากต้องการให้อินสแตนซ์ข้ามบัญชีสามารถอ่านไฟล์ที่เข้ารหัสได้ ต้องระบุการยอมรับสิทธิ์ที่หน้า **KMS Key Policy** ของกุญแจนั้นด้วย (Key Policy คือประตูด่านแรกของ KMS)
...
*   **Network Security & Firewalls:**
    *   **AWS WAF (Web Application Firewall):** บล็อกภัยคุกคามหน้าเว็บระดับแอปพลิเคชัน (Layer 7) ป้องกันการสแกน SQL Injection, Cross-Site Scripting (XSS), บล็อกตาม IP ประเทศ หรือจำกัดปริมาณการคลิกป่วน (Rate Limiting) สามารถนำไปเปิดใช้งานกับ CloudFront, ALB, API Gateway หรือ AppSync
    *   **AWS Shield:**
        *   *Shield Standard:* บริการฟรี ป้องกันภัย DDoS ระดับ Layer 3/4 (เช่น SYN Floods) ให้บริการอัติโนมัติแก่ทุกเครื่อง
        *   *Shield Advanced:* บริการรายเดือนราคาแพง ป้องกันภัยระดับสูง มีทีมผู้เชี่ยวชาญช่วยรบแก้ต่าง DDoS 24/7 (DRT team) และชดเชยค่าบริการส่วนเกินที่เกิดจากการโจมตียิงทราฟฟิกพังระบบ
...
*   **Threat Detection & Vulnerability Scanning:**
    *   **AWS GuardDuty:** ปัญญาประดิษฐ์ (ML) วิเคราะห์หาความผิดปกติของพฤติกรรมและการโจมตีระบบโดยคอยแอบอ่านจาก Log สำคัญ (VPC Flow Logs, DNS Query Logs, CloudTrail Logs) โดยไม่สร้างโหลดให้กับระบบประมวลผลเซิร์ฟเวอร์
    *   **AWS Inspector:** เครื่องมือแสกนหาช่องโหว่ความปลอดภัย (Vulnerability) และความสอดคล้องนโยบายของระบบปฏิบัติการเซิร์ฟเวอร์ EC2, ภาพอิมเมจใน ECR และฟังก์ชัน Lambda
    ...
    *   **AWS Security Hub:** บัญชีแดชบอร์ดส่วนกลางรวบรวมข้อมูลสถานะและการผ่านเกณฑ์ความปลอดภัยของทรัพยากรทั้งหมดใน AWS
...
*   **Identity Federation & Directory Services:**
    *   **AWS Directory Service:**
        *   *Managed Microsoft AD:* ติดตั้งและรัน AD จริงบน AWS เหมาะกับแอปที่พึ่งพาการตั้งค่า AD
    ...
    *   **Cognito:**
        *   **User Pools (CUP):** บริการระบบสมัครสมาชิก ยืนยันตัวตน และลงชื่อเข้าสู่หน้าแอปพลิเคชัน (Authentication) สนับสนุนการเข้าใช้งานผ่าน Social Login (Facebook, Google, Apple) หรือระบบ Enterprise (SAML/OIDC)
        *   **Identity Pools (Federated Identities):** นำโทเค็นที่ยืนยันตนจาก User Pools มาแลกรับชุดกุญแจยืนยันสิทธิ์ความปลอดภัยชั่วคราวของ AWS (STS credentials) เพื่อให้อุปกรณ์หรือไคลเอ็นต์สามารถดึงไฟล์จาก S3 หรือเซฟข้อมูลใน DynamoDB ได้โดยตรงโดยไม่ต้องมีแอปพลิเคชันหลังบ้านคั่นกลาง (Authorization)
*   **Compliance Tools:**
...
*   **Trap Warnings:**
    *   *WAF vs Shield:* หากเป้าหมายคือสกัดกั้นทราฟฟิกหน้าเว็บ SQL Injection, XSS, HTTP flood, หรือคัดกรองพฤติกรรมการเรียกเว็บ ให้ตอบ WAF; หากเป็นสัญญานยิงถล่มระดับเน็ตเวิร์กทราฟฟิกขนาดใหญ่ (DDoS Layer 3/4) ให้ตอบ Shield Advanced
...
*   **SaaS App Mobile Login:** สร้างแอปพลิเคชันมือถือโดยใช้งาน Cognito User Pools ในการทำระบบสมัครสมาชิกและล็อกอิน จากนั้นผู้ใช้ที่ผ่านเข้าระบบสำเร็จจะได้รับสิทธิ์ชั่วคราวจาก Cognito Identity Pools เพื่ออัปโหลดไฟล์ภาพโปรไฟล์ของตัวเองขึ้นสู่ S3 Folder ส่วนตัวได้อย่างปลอดภัย
...
2.  CloudFront ถูกเชื่อมโยงเข้ากับ **AWS WAF** เพื่อสกัดภัยคุกคาม ล้างข้อมูล SQL Injection และประเมินประเทศปลายทางของไอพีบราวเซอร์
3.  คำขอที่ปลอดภัยจะถูกส่งต่อเข้าสู่ระบบหลังบ้าน **Application Load Balancer (ALB)** และส่งต่อเข้าเครื่อง **EC2 (Web Server)**
4.  **Amazon GuardDuty** แอบคอยมอนิเตอร์ Log ขาเข้าผ่าน VPC Flow Logs เพื่อตรวจสอบว่าระบบไม่เจอบล็อกข้อมูลสั่งสแกนเครือข่ายแปลกๆ หรือแอบเจาะพอร์ตหลังบ้าน
5.  หากตรวจพบสิ่งผิดปกติในระดับวิกฤต GuardDuty จะส่งสัญญาณเหตุการณ์ไปยัง **Amazon EventBridge** เพื่อไปกระตุ้น **AWS Lambda** ให้เปลี่ยนค่า SG เพื่อบล็อกไอพีที่โจมตีเข้ามาทันที (Auto-remediation)
